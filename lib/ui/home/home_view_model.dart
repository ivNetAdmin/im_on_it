...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<void> editTask(BuildContext context, Task task) async {
    if(context.mounted) {
      SnackBarHelper.showFlashError(context, 'task edit', 'info');
    }
    _newTask = Task(
      id: task.id,
  ...
  Future<void> completeTask(BuildContext context, Task task) async {
    ...
    switch(result)
    {
      case Ok<int>():
        if(context.mounted) {
          SnackBarHelper.showFlashError(context, 'task completed', 'info');
        }
        _load();
      case Error<int>():
        if(context.mounted) {
  ...
  Future<void> deleteTask(BuildContext context, Task task) async {
    ...
    switch(result)
    {
      case Ok<int>():
        if(context.mounted) {
          SnackBarHelper.showFlashError(context, 'task deleted', 'info');
        }
        _load();
      case Error<int>():
        if(context.mounted) {
